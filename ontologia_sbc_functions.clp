...
;;; Funcion para hacer una pregunta con rango numerico
(deffunction MAIN::pregunta-rango (?pregunta ?rangini ?rangfi)
	(format t "%s [%d, %d] " ?pregunta ?rangini ?rangfi)
	(printout t "Min: ")
	(bind ?min (read))
	(printout t "Max: ")
	(bind ?max (read))
	(while (not(and(< ?min ?max) (and(>= ?min ?rangini)(<= ?max ?rangfi)))) do
		(format t "%s [%d, %d] " ?pregunta ?rangini ?rangfi)
		(printout t "Min: ")
		(bind ?min (read))
		(printout t "Max: ")
		(bind ?max (read))
		?rango <- (Rango (minimo ?min) (maximo ?max))
	)
	?rango
)
